...
const BUILD_ARTIFACTS = [
  ...
  '.turbo', '.parcel-cache', '.cache', 'coverage', '.nyc_output', 'out',
]
...
export function CleanupView() {
  ...
  const doClean = useCallback(() => {
    showConfirm('Git Clean', 'Execute git clean -fd? This will permanently remove untracked files and directories.', () => {
      doCleanRepo()
      setDryRunOutput([])
    })
  ...
  const doRemoveArtifacts = useCallback(() => {
    ...
    showConfirm('Remove Build Artifacts', `Remove ${found.length} director${found.length === 1 ? 'y' : 'ies'}? ${found.join(', ')}`, async () => {
      try {
  ...
  useInput((input, key) => {
    ...
    if (input === 'd') { doDryRun(); return }
    if (input === 'c') { doClean(); return }
    if (input === 'r') { doRemoveArtifacts(); return }
  })
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      ...
      <Box flexDirection="column" marginBottom={1}>
        <Text><Text bold color="yellow">d</Text> - Preview files that would be removed (dry run)</Text>
        <Text><Text bold color="red">c</Text> - Execute git clean (remove untracked files)</Text>
        <Text><Text bold color="red">r</Text> - Remove common build artifacts (node_modules, dist, etc.)</Text>
      </Box>
      ...
      {dryRunOutput.length > 0 && (
        <Box flexDirection="column" marginBottom={1} borderStyle="single" borderColor="gray" paddingX={1}>
          ...
          {dryRunOutput.map((line, i) => <Text key={i} color="yellow">{line}</Text>)}
        </Box>
      )}
      ...
      {foundArtifacts.length > 0 && (
        <Box flexDirection="column" marginBottom={1} borderStyle="single" borderColor="gray" paddingX={1}>
          ...
          {foundArtifacts.map((dir) => <Text key={dir} color="red">{dir}/</Text>)}
