  ...
  const actionItems = useMemo(
    (): ActionItem[] => [
      { key: 'f', label: 'Fetch all', action: () => doFetchAll() },
      {
        ...
        action: async () => {
          try {
            ...
            showNotification('error', 'Failed to open PR URL')
          }
        },
      },
      ...
      { key: 's', label: 'Stashes', action: () => setCurrentView('stashes') },
      {
        ...
        action: () =>
          selectedBranch &&
          showConfirm('Delete Branch', `Delete local branch "${selectedBranch.name}"?`, () =>
            doDeleteBranch(selectedBranch.name),
          ),
      },
      {
        key: 'D',
        ...
        action: () => {
          ...
          else if (selectedBranch.trackingGone)
            showNotification('error', `Remote branch "${selectedBranch.tracking}" is already gone`)
          else
            showConfirm(
              'Delete Remote',
              `Delete remote branch "${selectedBranch.tracking}"? This cannot be undone.`,
              () => doDeleteRemoteBranch(selectedBranch.tracking!),
            )
        },
      },
    ],
  ...
  useInput((input, key) => {
    ...
    if (key.return && selectedBranch) {
      if (selectedBranch.worktreePath && !selectedBranch.isCurrent) {
        showNotification(
          'error',
          `Branch "${selectedBranch.name}" is checked out in worktree at ${selectedBranch.worktreePath}`,
        )
        return
      }
      if (status.length > 0) {
        showConfirm(
          'Checkout',
          `You have ${status.length} uncommitted change(s). Checkout "${selectedBranch.name}" anyway?`,
    ...
    if (input === 'w' && selectedBranch) {
      if (selectedBranch.worktreePath) {
        ...
        showInput(
          'New Branch (optional)',
          `Leave empty to use "${selectedBranch.name}", or enter new branch name`,
          (branch: string) => {
            if (branch && INVALID_BRANCH_RE.test(branch)) {
    ...
    if (input === 'n' && selectedBranch) {
      showInput('Create Branch', `New branch name (from ${selectedBranch.name})`, (name: string) =>
        doCreateBranch(name, selectedBranch.name),
      )
      return
    }
    if (input === 'l') {
            ...
            <>
              ...
              <Hint
                ...
                keys={[
                  ...
                  { key: 'l', label: 'log' },
                  ...(selectedBranch?.isCurrent && !showLog && status.length > 8
                    ? [{ key: 'e', label: 'expand' }]
                    : []),
                  { key: '?', label: 'help' },
                  { key: 'q', label: 'quit' },
