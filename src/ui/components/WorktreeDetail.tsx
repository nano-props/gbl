...
export function WorktreeDetail({ worktree }: WorktreeDetailProps) {
  return (
    <Box marginTop={1} flexDirection="column" borderStyle="single" borderColor="gray" paddingX={1}>
      <Box><Text bold color="cyan">Path:   </Text><Text>{worktree.path}</Text></Box>
      <Box><Text bold color="cyan">Branch: </Text><Text>{worktree.branch || '(detached)'}</Text></Box>
      <Box><Text bold color="cyan">Commit: </Text><Text color="yellow">{worktree.head.slice(0, 7)}</Text></Box>
      <Box>
