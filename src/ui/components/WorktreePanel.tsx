...
export function WorktreePanel({ branch, worktrees, onClose }: WorktreePanelProps) {
  ...
  useInput((input, key) => {
    ...
    if (input === 'n') {
      showInput('New Branch', `New branch name (from ${branch.name})`, (newBranch: string) => {
        if (INVALID_BRANCH_RE.test(newBranch)) { showNotification('error', 'Invalid branch name'); return }
    ...
    if (input === 'd' && wt && !wt.isMainWorktree) {
      if (wt.isLocked) { showNotification('error', 'Worktree is locked'); return }
      if (wt.isDirty) {
        showConfirm('Remove Dirty Worktree', `Worktree has uncommitted changes. Force remove?`, () => { doRemoveWorktree(wt.path, true); onClose() })
      } else {
        showConfirm('Remove Worktree', `Remove worktree at "${wt.path}"?`, () => { doRemoveWorktree(wt.path); onClose() })
      }
      return
    }
  ...
  return (
    <Box marginTop={1} flexDirection="column" borderStyle="single" borderColor="magenta" paddingX={1}>
      <Text bold color="magenta">Worktree for "{branch.name}"</Text>
      {wt ? (
        <>
          <Box><Text bold color="cyan">Path:   </Text><Text>{wt.path}</Text></Box>
          <Box><Text bold color="cyan">Commit: </Text><Text color="yellow">{wt.head.slice(0, 7)}</Text></Box>
          <Box>
          ...
          {wt.statusFiles && wt.statusFiles.length > 0 && (
            <Box flexDirection="column" marginTop={1}>
              <Text bold color="cyan">Changes ({wt.statusFiles.length}):</Text>
              {wt.statusFiles.map((f, i) => {
                const label = statusLabel(f.x, f.y)
                return (
                  <Text key={i}><Text color={label.color}> {label.text} </Text><Text>{shortenPath(f.path)}</Text></Text>
                )
              })}
            </Box>
          )}
        ...
        <Box><Text bold color="cyan">Path:   </Text><Text>{branch.worktreePath}</Text></Box>
      )}
    </Box>
  )
}
