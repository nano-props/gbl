...
export async function stashSave(message?: string): Promise<ExecResult> {
  const args = ['stash', 'push']
  if (message) args.push('-m', message)
  return gitResult(...args)
}

export async function stashPop(index?: number): Promise<ExecResult> {
  const args = ['stash', 'pop']
  if (index !== undefined) args.push(`stash@{${index}}`)
  return gitResult(...args)
}

export async function stashDrop(index: number): Promise<ExecResult> {
